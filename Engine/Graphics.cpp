...
void Graphics::DrawCircumference(int x, int y, int radius, int border, Color c)
{
	...
	for (int j{ y - radius + 1 }; j < y + radius; j++)
		for (int i{ x - radius + 1 }; i < x + radius; i++)
		{
			...
			const bool condition = 
				(xDiff * xDiff + yDiff * yDiff > squareRadius - border) &&
				(xDiff * xDiff + yDiff * yDiff < squareRadius + border);
...
{
	for (int j{ y0 }; j < y1 && j < ScreenHeight; j++)
		for (int i{ x0 }; i < x1 && i < ScreenWidth; i++)
		{
			PutPixel(i, j, c);
		}
}
